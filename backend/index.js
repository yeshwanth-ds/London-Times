...
cron.schedule('51 23 * * *', async () => {  // Runs every day at 10:30 PM
  console.log("Cron job started: Fetching upcoming estimated orders...");
  try {
    const apiUrl = process.env.API_BASE_URL || 'http://localhost:5000/api/watch'; // Default to local API for dev
    const response = await axios.get(`${apiUrl}/getUpcomingEstimatedOrders`);
    console.log(apiUrl);
...
cron.schedule('55 23 * * *', async () => {  // Runs every 1st day of the month at 10:30 PM
  console.log("Cron job started: Fetching delivered watches...");
  try {
    const apiUrl = process.env.API_BASE_URL || 'http://localhost:5000/api/watch'; // Default to local API for dev
    const response = await axios.get(`${apiUrl}/deliveredWatches`);
